...
use ethers_core::types::{Address, Bytes, Transaction, H160, H256, I256, U256};
...
use crate::rlp::rlc::{RlcContextPair, RlcTrace, FIRST_PHASE};
use crate::rlp::{RlpArrayTraceWitness, RlpChip, RlpFieldWitness};
use crate::transaction::{get_transaction_type, EIP_1559_TX_TYPE_FIELDS_ITEM, EIP_2718_TX_TYPE};
...
use crate::{EthChip, EthCircuitBuilder, Network, ETH_LOOKUP_BITS};
...
impl ZkSyncTransactionsInput {
    pub fn assign<F: Field>(self, ctx: &mut Context<F>) -> ZkSyncTransactionsInputAssigned<F> {
        ...
        let amount_slots = self
            .amount_slots
            .into_iter()
            .map(|(token_address, pre_amount_slot, now_amount_slot)| {
                (
                    load_bytes(&token_address.0.to_vec()),
                    load_bytes(&pre_amount_slot.0.to_vec()),
                    load_bytes(&now_amount_slot.0.to_vec()),
                )
            })
            .collect_vec();
...
impl ZkSyncBlockTransactionCircuit {
    pub fn from_provider(provider: &Provider<Http>, tx_hash: H256, network: Network) -> Self {
        let inputs = get_zksync_transaction_and_storage_input(provider, tx_hash);
    ...
    pub fn create_circuit<F: Field>(
        ...
        let EIP1186ResponseDigest { index, slots_values } = digest;

        let assigned_instances = slots_values.into_iter().chain([index]).collect_vec();
        ...
        let circuit = EthCircuitBuilder::new(
            ...
            move |builder: &mut RlcThreadBuilder<F>,
                ...
                let _trace = chip
                    .parse_transaction_and_slot_proof_from_block_of_zksync_phase1(builder, witness);
            },
        );
...
impl<'chip, F: Field> ZkSyncBlockTransactionChip<F> for EthChip<'chip, F> {
    ...
    fn parse_transaction_and_slot_proof_from_block_of_zksync_phase0(
        ...
        // drop ctx
        let transactions_witness =
            self.parse_eip1186_proof_of_zksync_phase0(thread_pool, keccak, input);
    ...
    fn parse_eip1186_proof_of_zksync_phase0(
        ...
        let ctx = thread_pool.main(FIRST_PHASE);
        self.parse_transaction_and_slot_proof_of_zksync_phase0(ctx, keccak, transactions_and_slots)
    }
    ...
    fn parse_transaction_and_slot_proof_of_zksync_phase0(
        ...
        let from_validate_hash_bytes = transactions_and_slots
            .from_input
            .txs
            .get(from_validate_index as usize)
            .unwrap()
            .clone();

        let to_validate_index = bytes_to_u8(&transactions_and_slots.to_input.validate_index);
        let to_validate_hash_bytes =
            transactions_and_slots.to_input.txs.get(to_validate_index as usize).unwrap().clone();

        // verify that from_validate_hash_bytes is equal to to_validate_hash_bytes
        for (from_validate_hash, to_validate_hash) in
            from_validate_hash_bytes.iter().zip(to_validate_hash_bytes.iter())
        {
        ...
        let mut get_transaction_info = |transaction: &AssignedBytes<F>| {
            ...
            let decode_transaction =
                Transaction::decode(&Rlp::new(&decode_transaction_rlp_u8)).unwrap();
        ...
        let (validate_from, validate_to, _, _) =
            get_transaction_info(&from_validate_hash_bytes.clone());
        let mut get_tx_witness = |txs: Vec<AssignedBytes<F>>, effective_address: Address| {
            ...
            let txs_witness: Vec<RlpArrayTraceWitness<F>> = txs
                .iter()
                .map(|transaction| {
                    let (transaction_from, transaction_to, token_address, token_value) =
                        get_transaction_info(&transaction.clone());
                    ...
                    } else if transaction_to.eq(&effective_address) {
                        // amount+
                    ...
                    if transaction_type != EIP_2718_TX_TYPE {
                        ...
                        let dest_value_bytes = get_transaction_field_rlp(
                            transaction_type,
                            &non_prefix_bytes_u8,
                            12,
                            EIP_1559_TX_TYPE_FIELDS_ITEM,
                        );
        ...
        let (from_txs_witness, from_effective_txs_witness, from_amount_map) =
            get_tx_witness(transactions_and_slots.from_input.txs.to_vec(), validate_from);
        let (to_txs_witness, to_effective_txs_witness, to_amount_map) =
            get_tx_witness(transactions_and_slots.to_input.txs.to_vec(), validate_to);

        // compute nonce from big-endian bytes
        let from_nonce_pre = bytes_be_to_uint(
            ctx,
            self.gate(),
            &transactions_and_slots.from_input.nonce_slots.0,
            32,
        );
        let from_nonce_now = bytes_be_to_uint(
            ctx,
            self.gate(),
            &transactions_and_slots.from_input.nonce_slots.1,
            32,
        );
        let from_nonce_diff = self.gate().sub(ctx, from_nonce_now, from_nonce_pre);
        let to_nonce_pre =
            bytes_be_to_uint(ctx, self.gate(), &transactions_and_slots.to_input.nonce_slots.0, 32);
        let to_nonce_now =
            bytes_be_to_uint(ctx, self.gate(), &transactions_and_slots.to_input.nonce_slots.1, 32);
        ...
        let mut equal_tx_nonce_to_expect =
            |txs_witness: &Vec<RlpArrayTraceWitness<F>>, nonce_pre: AssignedValue<F>| {
        ...
        let mut get_amount_map_slots = |amount_map: HashMap<H160, I256>| {
            let amount_map_slot: Vec<(AssignedBytes<F>, AssignedValue<F>, AssignedValue<F>)> =
                amount_map
                    .iter()
                    .map(|(token_address, token_value)| {
                        let token_address = load_bytes(ctx, &*token_address.0.to_vec());
                        let negative = ctx.load_witness(F::from(token_value.is_negative()));
                        let token_value =
                            (F::from(token_value.unsigned_abs().as_u64())).try_into().unwrap();
        ...
        let mut equal_tx_amount_to_expect = |amount_slots: &Vec<(
            AssignedBytes<F>,
            AssignedBytes<F>,
            AssignedBytes<F>,
        )>,
                                             amount_map_slots: &Vec<(
            AssignedBytes<F>,
            AssignedValue<F>,
            AssignedValue<F>,
        )>| {
            let is_equals: Vec<bool> = amount_slots
                .iter()
                .map(|(token_address, amount_pre_block_slot, amount_now_block_slot)| {
                    let gt_or_lt =
                        bytes_to_vec_u8_gt_or_lt(amount_pre_block_slot, amount_now_block_slot);
                    let negative = if gt_or_lt.eq(&1) {
                        true
                    } else if gt_or_lt.eq(&-1) {
                        false
                    } else {
                        true
                    };
                    let amount_pre_block_slot =
                        bytes_be_to_uint(ctx, self.gate(), amount_pre_block_slot, 32);
                    let amount_now_block_slot =
                        bytes_be_to_uint(ctx, self.gate(), amount_now_block_slot, 32);
                    let amount_diff;
                    if negative {
                        amount_diff =
                            self.gate().sub(ctx, amount_pre_block_slot, amount_now_block_slot);
                    } else {
                        amount_diff =
                            self.gate().sub(ctx, amount_now_block_slot, amount_pre_block_slot);
                    };
                    ...
                    for (slot_token_address, slot_token_amount, slot_value_negative) in
                        amount_map_slots
                    {
        ...
        equal_tx_amount_to_expect(
            &transactions_and_slots.from_input.amount_slots,
            &from_amount_map_slots,
        );
        equal_tx_amount_to_expect(
            &transactions_and_slots.to_input.amount_slots,
            &to_amount_map_slots,
        );
    ...
    fn parse_transaction_and_slot_proof_from_block_of_zksync_phase1(
        ...
        let txs_trace =
            self.parse_eip1186_proof_of_zksync_phase1(thread_pool, witness.input_witness);
        ZkSyncBlockTransactionTrace { txs_trace }
    }
    ...
    fn parse_eip1186_proof_of_zksync_phase1(
        ...
        let mut get_max_len = |txs_witness: &Vec<RlpArrayTraceWitness<F>>| {
            let len: usize =
                txs_witness.into_iter().map(|tx_witness| tx_witness.rlp_array.len().pow(5)).sum();
            len
        };
        ...
        let zksync_transaction_trace =
            self.parse_transaction_and_slot_proof_of_zksync_phase1((ctx_gate, ctx_rlc), witness);
    ...
    fn parse_transaction_and_slot_proof_of_zksync_phase1(
        ...
        ZkSyncTransactionTrace { from_txs_trace, to_txs_trace }
    }
}
