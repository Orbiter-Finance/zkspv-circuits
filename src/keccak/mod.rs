...
impl<F: Field> KeccakChip<F> {
    ...
    pub fn keccak_fixed_len(
        ...
        // let expect_hash = hex::encode(output_bytes);
        // println!("expect_hash:{}", expect_hash);
    ...
    FnPhase1: FnSynthesize<F>,
{
    ...
    pub fn config(&self, k: usize, minimum_rows: Option<usize>) -> EthConfigParams {
        ...
        println!("keccak.capacity():{}", keccak.capacity());
        println!("minimum_rows: {:?}", minimum_rows.unwrap_or(0));
        let optimal_rows_per_round =
            rows_per_round((1 << k) - minimum_rows.unwrap_or(0), keccak.capacity());//2^k - minimum_rows.unwrap_or(0)
        println!("optimal_rows_per_round: {}", optimal_rows_per_round);
        ...
        println!("keccak_rows_per_round: {}", params.keccak_rows_per_round);
