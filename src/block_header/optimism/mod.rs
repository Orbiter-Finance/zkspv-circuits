...
use halo2_base::{
    AssignedValue,
    Context,
    gates::{builder::GateThreadBuilder, GateInstructions, RangeChip}, QuantumCell::{Constant, Existing},
    utils::bit_length,
};
...
use crate::{ETH_LOOKUP_BITS, EthChip, EthCircuitBuilder, EthereumNetwork, Field, keccak::{FixedLenRLCs, FnSynthesize, KeccakChip, VarLenRLCs}, Network, rlp::{
...
use crate::util::helpers::{get_block_header_rlp_max_bytes, get_optimism_block_header_rlp_max_field_lens};
...
const OPTIMISM_MAINNET_EXTRA_DATA_MAX_BYTES: usize = 97;
const OPTIMISM_MAINNET_EXTRA_DATA_RLP_MAX_BYTES: usize = OPTIMISM_MAINNET_EXTRA_DATA_MAX_BYTES + 1;
const OPTIMISM_GOERLI_EXTRA_DATA_MAX_BYTES: usize = 32;
const OPTIMISM_GOERLI_EXTRA_DATA_RLP_MAX_BYTES: usize = OPTIMISM_GOERLI_EXTRA_DATA_MAX_BYTES + 1;

/// This is the minimum possible RLP byte length of a block header *at any block* (including pre EIPs)
const OPTIMISM_BLOCK_HEADER_RLP_MIN_BYTES: usize = 479;
...
pub const OPTIMISM_MAINNET_BLOCK_HEADER_RLP_MAX_BYTES: usize =
    1 + 2 + (521 + OPTIMISM_MAINNET_EXTRA_DATA_RLP_MAX_BYTES + 33); // 33 is for withdrawals_root
pub const OPTIMISM_GOERLI_BLOCK_HEADER_RLP_MAX_BYTES: usize =
    1 + 2 + (521 + OPTIMISM_GOERLI_EXTRA_DATA_RLP_MAX_BYTES + 33);

const OPTIMISM_NUM_BLOCK_HEADER_FIELDS: usize = 17;
pub const OPTIMISM_MAINNET_HEADER_FIELDS_MAX_BYTES: [usize; OPTIMISM_NUM_BLOCK_HEADER_FIELDS] =
    [32, 32, 20, 32, 32, 32, 256, 7, 4, 4, 4, 4, OPTIMISM_MAINNET_EXTRA_DATA_MAX_BYTES, 32, 8, 6, 32];
pub const OPTIMISM_GOERLI_HEADER_FIELDS_MAX_BYTES: [usize; OPTIMISM_NUM_BLOCK_HEADER_FIELDS] =
    [32, 32, 20, 32, 32, 32, 256, 7, 4, 4, 4, 4, OPTIMISM_GOERLI_EXTRA_DATA_MAX_BYTES, 32, 8, 6, 32];
/// The maximum number of bytes it takes to represent a block number, without any RLP encoding.
pub const OPTIMISM_BLOCK_NUMBER_MAX_BYTES: usize = OPTIMISM_MAINNET_HEADER_FIELDS_MAX_BYTES[8];

// Field                        Type            Size (bytes)    RLP size (bytes)    RLP size (bits)
// parentHash	                256 bits	    32	            33	                264
// UncleHash	                256 bits	    32	            33	                264
...
impl<'chip, F: Field> EthBlockHeaderChip<F> for EthChip<'chip, F> {
    fn decompose_block_header_phase0(
        ...
        let max_field_lens = get_optimism_block_header_rlp_max_field_lens(&network);
        ...
        let block_hash_query_idx = keccak.keccak_var_len(
            ...
            rlp_witness.rlp_len,
            OPTIMISM_BLOCK_HEADER_RLP_MIN_BYTES,
        );
    ...
    fn decompose_block_header_phase1(
        ...
        let [parent_hash, ommers_hash, beneficiary, state_root, transactions_root, receipts_root, logs_bloom, difficulty, number, gas_limit, gas_used, timestamp, extra_data, mix_hash, nonce, basefee, withdrawals_root]: [RlpFieldTrace<F>; OPTIMISM_NUM_BLOCK_HEADER_FIELDS] =
...
pub fn get_boundary_block_data<F: Field>(
    ...
    let block_numbers = {
        debug_assert_eq!(chain[0].get("number").max_field_len, OPTIMISM_BLOCK_NUMBER_MAX_BYTES);
        let start_block_number_bytes = bytes_be_var_to_fixed(
            ctx,
            gate,
            &chain[0].get("number").field_cells,
            chain[0].get("number").field_len,
            OPTIMISM_BLOCK_NUMBER_MAX_BYTES,
        );
        // TODO: is there a way to do this without so many selects
        let end_block_number_bytes: [_; OPTIMISM_BLOCK_NUMBER_MAX_BYTES] =
