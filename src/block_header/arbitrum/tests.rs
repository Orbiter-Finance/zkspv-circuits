use crate::{ArbitrumNetwork, EthereumNetwork, keccak::SharedKeccakChip, util::{EthConfigPinning, Halo2ConfigPinning}};
...
pub fn test_one_arbitrum_goerli_header_mock() {
    ...
    input_bytes.resize(ARBITRUM_GOERLI_BLOCK_HEADER_RLP_MAX_BYTES, 0);
...
pub fn test_one_arbitrum_mainnet_header_mock() {
    ...
    let mut input_bytes: Vec<u8> = Vec::from_hex(input_hex).unwrap();
    input_bytes.resize(ARBITRUM_MAINNET_BLOCK_HEADER_RLP_MAX_BYTES, 0);
...
fn get_default_goerli_header_chain_circuit() -> EthBlockHeaderChainCircuit<Fr> {
    ...
    let header_rlp_max_bytes = ARBITRUM_GOERLI_BLOCK_HEADER_RLP_MAX_BYTES;
...
mod aggregation {
    ...
    use crate::block_header::arbitrum::helpers::{BlockHeaderScheduler, CircuitType, Finality, Task};
