use std::{path::Path, fmt::format, ops::Range};
...
use crate::{
    ...
    transaction::ethereum::{helper::TransactionTask as ETHTransactionTask, EthBlockTransactionCircuit}, 
    Network, track_block::{EthTrackBlockCircuit, util::get_eth_track_block_circuit}, EthereumNetwork
};
...
impl scheduler::Task for ArbitrationTask {
    ...
    fn circuit_type(&self) -> Self::CircuitType {
        match self {
            ArbitrationTask::ETHBlockTrack(task) => ArbitrationCircuitType::TrackBlock(task.circuit_type()),
    ...
    fn dependencies(&self) -> Vec<Self> {
        match self {
            ArbitrationTask::Transaction() => todo!(),
            ArbitrationTask::MDCState(_) => todo!(),
            ArbitrationTask::ETHBlockTrack(task) => task.dependencies().into_iter().map(ArbitrationTask::ETHBlockTrack).collect(),
            ArbitrationTask::Final(_) => todo!(),
        }
    }
}