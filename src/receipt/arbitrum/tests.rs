...
use crate::{ Network,ArbitrumNetwork};
use crate::receipt::arbitrum::EthBlockReceiptCircuit;
...
pub fn test_receipt_mpt() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let input = get_test_circuit(receipt_index, receipt_rlp, merkle_proof, Network::Arbitrum(ArbitrumNetwork::Goerli));
